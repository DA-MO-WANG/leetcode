...
int find(int x) {
    if(f[x] != x) {
        //在父节点递归完成时，父节点的hg[父]也完成了，已经更新成了到根节点的距离
        int t = find(f[x]);
        //更新当前节点到根节点的距离
        hg[x] += hg[f[x]];
        //更新当前节点到根节点的路径压缩
...
int main() {
    ...
    while (k--) {
        ...
        if(x > n || y > n) ++res;
            ...
            if (d == 1) {
                if(zx == zy) {
                    //也存在隔代同类，是否同类依赖食物链关系，而不是单纯的距离
                    //食物链关系体现在距离对3取模的差上
                    if((hg[x] - hg[y])%3)  ++res;//只有距离取模后差在0，才属于同类--包含了隔代类情况-周期情况
                }else {
                ...
                if(d == 2) {
                    if(zx == zy) {
                        //只有距离取模差，差在1上，再取模3，不为0就是这个贪吃关系，包含了1吃0，2吃1，0吃2
                        if((hg[x] - hg[y] - 1)%3) ++res;
                    }else {
                        f[zx] = zy;
                        hg[zx] = hg[y] + 1 - hg[x];
                    }
